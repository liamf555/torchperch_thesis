...
class BixlerEnvContRate(gym.Env):
    metadata = {'render.modes': ['file', 'none']}

    def __init__(self, controller='sweep_elevator_cont_rate',
			scenario='perching',
			scenario_opts=''):
        ...
        self.action_space = gym.spaces.Box(low = np.array([self.bixler.sweep_rates[0], self.bixler.elev_rates[0]]),
                                            high = np.array([self.bixler.sweep_rates[1], self.bixler.elev_rates[1]]), dtype = np.float16)
