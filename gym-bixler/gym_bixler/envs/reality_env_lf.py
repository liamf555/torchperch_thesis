...
class RealBixlerEnv(gym.EnV):
	...
	def step(self,action):
		...
		real_action = model.get_env().%handwaving%.controller.convert_action(action)
    	self.master.mav.mlagent_action_send(1,1,real_action[0,0],real_action[0,1])

		# Get observation
		obs = self._wait_for_MLAGENT_STATE()

		# TODO: Get reward

		reward = scenario.get_reward(obs)
	...
	def reset(self):
		...
		return self._wait_for_MLAGENT_STATE() 
