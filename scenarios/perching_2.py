...
def wrap_class(BixlerClass, parameters):
    class PerchingBixler(BixlerClass):
                ...
                def get_normalized_obs(self, state=None):
                    ...
                    mins = np.array([ -50,  h_min,  -pb2, 0, -5, -pb2,  self.sweep_limits[0], self.elev_limits[0], 0, 0])
                    maxs = np.array([  10,    1,     pb2, 20, 5, pb2,  self.sweep_limits[1], self.elev_limits[1], 25, 25])
