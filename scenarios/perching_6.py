...
def wrap_class(BixlerClass, parameters):
    class PerchingBixler(BixlerClass):
                ...
                def get_normalized_obs(self, state=None):
                    ...
                    obs = np.float64(np.concatenate((obs, [[self.wind[0], self.wind[2], self.Vr[0], self.Vr[2], self.velocity_e[0], self.velocity_e[2]]]), axis = 1))

                    pb2 = 2*np.pi

                    mins = np.array([ -50,  h_min,  -pb2, -10, -10, -pb2,  self.sweep_limits[0], self.elev_limits[0], -3.5, -3.5,   -10,   -10,   -10,  -10])
                    maxs = np.array([  10,    1,     pb2, 20, 10, pb2,  self.sweep_limits[1], self.elev_limits[1],   8.5,   8.5, 25, 25, 20, 20])
