...
def get_run(bixler,model):
    state_history = []
    
    while not bixler.is_terminal():

        bixler_state = np.concatenate( (bixler.get_state().T, np.array(bixler.throttle,ndmin=2) ), axis=0 )
...
def plot_states(axs,states,controller):
...
if __name__ == "__main__":
    ...
    if args.stdout:
        for timestep in states[:]:
            print( ','.join(map(str,timestep[:,0])) )
    
    if args.graph:
        fig, axs = plt.subplots(3,2)

        plot_states(axs,states,controller)
